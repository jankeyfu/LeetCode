...
func findInMountainArray(target int, mountainArr *MountainArray) int {
	...
	fmt.Println(l, r, top)
	idx := binarySearch(target, mountainArr, l, top, 1)
	if idx == -1 {
		fmt.Println("========")
		idx = binarySearch(target, mountainArr, top+1, r, -1)
	}
	return idx
}
...
func binarySearch(target int, mountainArr *MountainArray, l, r, sig int) int {
	target = sig * target
	for l <= r {
		mid := (l + r) / 2
		midv := sig * mountainArr.get(mid)
		fmt.Println(mountainArr.Arr, target, l, r, mid, midv)
		if midv == target {
			return mid
		} else if midv < target {
			l = mid + 1
...
func (this *MountainArray) get(index int) int { return this.Arr[index] }
func (this *MountainArray) length() int       { return len(this.Arr) }
